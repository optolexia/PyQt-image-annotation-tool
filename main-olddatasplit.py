...
class LabelerWindow(QWidget):
    def __init__(self, labels, input_folder, mode):
        ...
        self.img_panel_width = 1000
        self.img_panel_height = 1000

        # state variables
        self.counter = 0
        self.input_folder = input_folder
        self.img_df = get_img_df()
        self.labels = labels
        self.num_labels = len(self.labels)
        self.num_images = len(self.img_df)
    ...
    def init_ui(self):
        ...
        self.img_name_label.setText(
            ...
            + "\n"
            + str(self.img_df.iloc[self.counter]["frameTimestamp"])
        )
    ...
    def set_label(self, label):
        ...
        img_path = self.img_df[self.counter]
        img_name = self.img_df[self.counter]
    ...
    def set_image(self, counter):
        ...
        try:
            ...
            print("Something bad happened to the image")

    def decode_image_to_pixmap(self, counter):
        try:
            image_cv2 = cv2.imdecode(
                np.frombuffer(self.img_df.iloc[counter]["content"], np.uint8),
                cv2.IMREAD_COLOR,
            )
            w, h, ch = image_cv2.shape

            qimg = QImage(
                image_cv2.data, h, w, 3 * h, QImage.Format_RGB888
            ).rgbSwapped()
            return QPixmap(qimg)
        except:
            print("Decoding the image went wrong")
